      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-16">
          ...
          <div className="lg:col-span-5">
            ...
            >
              <span className="inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-semibold border mb-5" style={{ color: 'var(--color-accent)', borderColor: 'rgba(0,128,129,0.3)', backgroundColor: 'rgba(0,128,129,0.06)' }}>
              ...
              <h2 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-black leading-tight">
                An Integrated <br />
                <span style={{ color: 'var(--color-accent)' }}>Venture Ecosystem</span>
              </h2>
              ...
              <div className="relative mt-8 rounded-2xl p-5 border" style={{ backgroundColor: '#FFFFFF', borderColor: 'rgba(0,0,0,0.08)' }}>
                <Quote className="absolute -top-2 -left-2 h-6 w-6" style={{ color: 'var(--color-accent)', opacity: 0.2 }} />
                <p className="text-xs italic text-black/60 leading-relaxed pl-3 border-l-2" style={{ borderColor: 'var(--color-accent)' }}>
              ...
              <div className="grid grid-cols-3 gap-5 mt-10">
                {stats.map((stat) => (
                  <div key={stat.label} className="text-center">
                    <p className="text-3xl font-black leading-none" style={{ color: 'var(--color-accent)' }}>{stat.value}</p>
          ...
          <div className="lg:col-span-7 flex flex-col gap-6">
            ...
            >
              {/* Decorative corner */}
              <div className="absolute top-0 right-0 h-40 w-40 rounded-bl-[3rem] pointer-events-none opacity-30" style={{ background: 'linear-gradient(135deg, var(--color-accent), transparent)' }} />

              <div className="relative">
                <h3 className="text-sm font-bold text-black flex items-center gap-2.5 mb-6">
                  <span className="flex h-5 w-5 items-center justify-center rounded-md text-[10px] font-black text-white" style={{ backgroundColor: 'var(--color-accent)' }}>◉</span>
                  ...
                  {connections.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        style={isActive ? { backgroundColor: '#000000', borderColor: '#000000', color: '#FFFFFF' } : { backgroundColor: '#000000', borderColor: 'rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.6)' }}
                        onMouseEnter={(e) => { if (!isActive) { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.4)'; e.currentTarget.style.color = '#FFFFFF' } }}
                        onMouseLeave={(e) => { if (!isActive) { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.15)'; e.currentTarget.style.color = 'rgba(255,255,255,0.6)' } }}
                      >
                        <Icon className="h-3.5 w-3.5" style={{ color: isActive ? '#FFFFFF' : 'rgba(255,255,255,0.5)' }} />
                        {item.label}
                      </button>
                    )
                  })}
                ...
                {/* Active Panel */}
                <div className="mt-5 rounded-2xl p-5 border" style={{ backgroundColor: '#FFFFFF', borderColor: '#000000' }}>
                    ...
                    >
                      <div className="flex items-center justify-between gap-3 mb-3">
                        <div className="flex items-center gap-2.5">
                          <div className="h-8 w-8 rounded-xl flex items-center justify-center" style={{ backgroundColor: 'var(--color-accent-hover)' }}>
                        ...
                        <div className="text-right rounded-xl px-3 py-1.5 border" style={{ backgroundColor: '#FFFFFF', borderColor: '#000000' }}>
                          <span className="text-sm font-black text-black">{currentConnection.stat}</span>
                          <span className="text-[9px] block font-medium tracking-wide uppercase" style={{ color: 'var(--color-accent)' }}>{currentConnection.statLabel}</span>
                        </div>
                      </div>
                  ...
                  <div className="mt-4 pt-3 border-t border-black/10 flex items-center justify-between">
                    ...
                    <a href="#contact" className="text-[10px] font-bold flex items-center gap-1" style={{ color: 'var(--color-accent-hover)' }}>
            ...
            >
              <div className="absolute -inset-1 rounded-3xl blur-sm" style={{ backgroundColor: 'rgba(0,0,0,0.3)' }} />
              ...
              <div className="relative">
                <div className="flex items-center gap-3 mb-6 pb-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                  <span className="flex h-6 w-6 items-center justify-center rounded-lg text-[10px] font-bold" style={{ backgroundColor: 'rgba(0,128,129,0.15)', color: 'var(--color-accent)' }}>✦</span>
                ...
                <div className="grid sm:grid-cols-3 gap-4">
                  {[
                    ...
                    const Icon = item.icon
                    return (
                      <div key={i} className="rounded-2xl p-5 shadow-lg hover:-translate-y-0.5 transition-transform duration-200" style={{ backgroundColor: 'rgba(255,255,255,0.05)', border: '1px solid var(--color-accent)' }}>
                        <div className="flex h-8 w-8 items-center justify-center rounded-lg mb-3" style={{ backgroundColor: 'var(--color-accent)' }}>
                          <Icon className="h-4 w-4 text-white" />
                        </div>
                        <h4 className="text-sm font-extrabold text-white">{item.title}</h4>
                        <p className="text-[11px] mt-1.5 leading-relaxed" style={{ color: 'rgba(255,255,255,0.6)' }}>{item.text}</p>
                      </div>
                    )
                  })}
                </div>
