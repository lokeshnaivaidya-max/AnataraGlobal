...
export default function MfaVerify() {
  ...
  // Prefer purpose-specific mfaToken over legacy password storage
  ...
  const onSubmit = async (data: MfaVerifyFormData) => {
    ...
    setError('')
    try {
      const body: Record<string, string> = { email, code: data.code }
      if (mfaToken) {
      ...
      await fetch(`${import.meta.env.VITE_API_URL || '/api'}/auth/mfa/authenticate`, {
        ...
        completeLogin(json.token, json.refreshToken, json.user)
