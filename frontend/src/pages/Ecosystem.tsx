    ...
    <>
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16">
            ...
            <div>
              ...
              <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                ...
                Our Ecosystem<br />
                <span style={{ color: 'var(--color-accent)' }}>Model</span>
              </motion.h1>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }}
              ...
              <div className="absolute -top-10 -right-10 h-32 w-32 rounded-full blur-3xl" style={{ backgroundColor: 'rgba(0,128,129,0.08)' }} />
              <div className="absolute top-0 left-0 w-full h-px" style={{ background: 'linear-gradient(to right, transparent, var(--color-accent), transparent)' }} />
              ...
              <div className="flex gap-2 mb-6">
                {ecosystemSteps.map((step, idx) => (
                  <button key={step.title} onClick={() => setActiveStep(idx)}
                    className="flex-1 py-2 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-300 cursor-pointer flex items-center justify-start pl-4"
                    style={activeStep === idx ? { backgroundColor: 'rgba(0,128,129,0.15)', border: '1px solid rgba(0,128,129,0.3)', color: 'var(--color-accent)' } : { backgroundColor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.4)' }}
                ...
                <AnimatePresence mode="wait">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex h-14 w-14 items-center justify-center rounded-2xl border"
                      style={{ backgroundColor: 'rgba(0,128,129,0.12)', borderColor: 'rgba(0,128,129,0.2)', color: 'var(--color-accent)' }}>
      ...
      <section className="relative py-24 lg:py-32 overflow-hidden" style={{ backgroundColor: '#000000' }}>
        <div className="absolute inset-0">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] rounded-full blur-3xl" style={{ backgroundColor: 'rgba(0,128,129,0.05)' }} />
        </div>
        <div className="absolute top-0 left-0 w-full h-px" style={{ background: 'linear-gradient(to right, transparent, var(--color-accent), transparent)' }} />
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            <div className="flex items-center justify-center gap-3 mb-4">
              <span className="h-px w-8" style={{ backgroundColor: 'var(--color-accent-light)' }} />
              <span className="text-xs font-semibold tracking-widest uppercase" style={{ color: 'var(--color-accent-light)' }}>The Journey</span>
              <span className="h-px w-8" style={{ backgroundColor: 'var(--color-accent-light)' }} />
            </div>
                  ...
                  <motion.div key={step.title} variants={cardVariants} whileHover={{ y: -8 }} className="group relative text-center">
                    <div className="hidden lg:flex justify-center relative z-10 mb-8">
                      <div className="h-8 w-8 rounded-full border-2 shadow-lg flex items-center justify-center"
                        style={{ backgroundColor: '#000000', borderColor: isGold ? 'var(--color-accent-light)' : 'var(--color-accent)' }}>
                        <span className="h-3 w-3 rounded-full" style={{ backgroundColor: isGold ? 'var(--color-accent-light)' : 'var(--color-accent)' }} />
                      </div>
                    </div>

                    <div className="rounded-3xl p-7 shadow-md hover:shadow-2xl transition-all duration-500 h-full flex flex-col items-center"
                      style={{ backgroundColor: 'rgba(255,255,255,0.04)', border: '1px solid var(--color-accent)' }}>
                      <div className="flex h-16 w-16 items-center justify-center rounded-2xl border mb-5 transition-all duration-300 group-hover:scale-110"
                        style={{
                          backgroundColor: isGold ? 'var(--color-accent-light)' : 'var(--color-accent)',
                          borderColor: isGold ? 'var(--color-accent-light)' : 'var(--color-accent)',
                          color: '#FFFFFF',
                        }}>
                        <Icon className="h-8 w-8" />
                      </div>

                      <span className="text-[10px] font-black uppercase tracking-widest" style={{ color: isGold ? 'var(--color-accent-light)' : 'var(--color-accent)' }}>
                        {step.step}
                      </span>
                      ...
                      <ul className="mt-5 pt-5 w-full space-y-2" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
                        {step.points.map((point) => (
                          <li key={point} className="flex items-center justify-start gap-2 text-xs font-medium" style={{ color: 'rgba(255,255,255,0.7)' }}>
                            <CheckCircle2 className="h-3.5 w-3.5 shrink-0" style={{ color: isGold ? 'var(--color-accent-light)' : 'var(--color-accent)' }} />
                            {point}
                          </li>
                        ))}
                      </ul>
          ...
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="mt-20 text-center">
            <div className="inline-flex items-center gap-3 rounded-full px-6 py-3 text-[10px] sm:text-xs font-semibold whitespace-nowrap"
              style={{ backgroundColor: 'rgba(51,181,181,0.1)', border: '1px solid rgba(51,181,181,0.25)', color: 'var(--color-accent-light)' }}>
