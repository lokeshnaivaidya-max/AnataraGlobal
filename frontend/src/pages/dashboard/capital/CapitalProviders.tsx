        ...
        {filtered?.map((provider, i) => (
          ...
          >
            <div className="flex items-center gap-3 mb-3">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-gold/10 text-lg font-extrabold text-gold shrink-0">{provider.name?.charAt(0) || 'C'}</div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-deep-navy truncate">{provider.name}</p>
                <span className="rounded-full bg-light-gray text-medium-gray px-2.5 py-0.5 text-[10px] font-semibold">{(CAPITAL_PROVIDER_TYPES.find(t => t.value === provider.type) as any)?.label || provider.type}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 text-xs text-medium-gray mb-3">
              <span className="flex items-center gap-1"><DollarSign className="h-3.5 w-3.5 text-gold" />{provider.currency || '$'} {((provider.minAmount || 0) / 1000).toFixed(0)}K - {((provider.maxAmount || 0) / 1000).toFixed(0)}K</span>
