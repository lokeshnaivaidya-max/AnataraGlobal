...
import { Loader2, Save, Building2, CheckCircle2, AlertCircle } from 'lucide-react'
import {
  useStartup,
  useUpdateStartup,
  useBusinessStages,
...
export default function StartupPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error('Startup save failed:', err)
      setSaveStatus('error')
        ...
        <div className="flex items-center justify-between pt-2 gap-4">
          {saveStatus === 'success' && (
            <div className="flex items-center gap-2 rounded-xl bg-success/10 border border-success/20 px-4 py-2.5 text-sm font-semibold text-success">
              <CheckCircle2 className="h-4 w-4" /> Startup saved successfully!
            </div>
          )}
          {saveStatus === 'error' && (
