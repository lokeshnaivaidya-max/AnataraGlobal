      ...
      <div className="space-y-3">
        {filtered?.map((contact, i) => (
          <motion.div key={contact.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}>
            <Link href={`/dashboard/founder/investor-crm/${contact.id}`}
              className="flex items-start gap-4 rounded-2xl border border-border-gray bg-white p-5 hover:shadow-lg hover:border-gold/20 transition-all group"
            >
