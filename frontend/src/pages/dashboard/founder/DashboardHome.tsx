...
import { useFounderProfile, useStartup, useDocuments, useEducation, useExperience, useSkills } from '../../../lib/founder-hooks'
...
export default function DashboardHome() {
  ...
  // Each section = 20% — computed from real data
  const sectionDone = {
    profile: !!(founder?.bio || founder?.linkedinUrl || founder?.phone),
    startup: !!(startup?.name),
    team: !!(education?.length || experience?.length || skills?.length),
    documents: !!(documents?.length),
    kyc: founder?.kycStatus === 'submitted' || founder?.kycStatus === 'verified',
  }

  const completedCount = Object.values(sectionDone).filter(Boolean).length
  const completed = completedCount * 20

  const completionItems = [
    { key: 'profile' as const, label: 'Personal Profile', icon: User, href: '/dashboard/founder/profile' },
    { key: 'startup' as const, label: 'Startup Details', icon: Building2, href: '/dashboard/founder/startup' },
    { key: 'team' as const, label: 'Team Members', icon: Users, href: '/dashboard/founder/team' },
    { key: 'documents' as const, label: 'Documents', icon: FileText, href: '/dashboard/founder/documents' },
    { key: 'kyc' as const, label: 'KYC Verification', icon: ShieldCheck, href: '/dashboard/founder/kyc' },
  ]
      ...
      >
        ...
        <div className="space-y-3">
          {completionItems.map((item) => {
            ...
            const isComplete = sectionDone[item.key]
            return (
              <Link
                key={item.key}
                href={item.href}
