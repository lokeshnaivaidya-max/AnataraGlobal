...
import { Loader2, Plus, Trash2, Save, GraduationCap, Briefcase, Wrench, CheckCircle2, AlertCircle } from 'lucide-react'
import {
  useFounderProfile,
...
export default function ProfilePage() {
  const { data: founder, isLoading, refetch: refetchProfile } = useFounderProfile()
  const updateProfile = useUpdateFounderProfile()
  const { data: education, refetch: refetchEdu } = useEducation()
  const addEducation = useAddEducation()
  const deleteEducation = useDeleteEducation()
  const { data: experience, refetch: refetchExp } = useExperience()
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      >
        ...
        {saveStatus === 'success' && (
          <div className="flex items-center gap-2 rounded-xl bg-success/10 border border-success/20 px-4 py-2.5 text-sm font-semibold text-success">
            <CheckCircle2 className="h-4 w-4" /> Profile saved successfully!
          </div>
        )}
        {saveStatus === 'error' && (
          <div className="flex items-center gap-2 rounded-xl bg-error/10 border border-error/20 px-4 py-2.5 text-sm font-semibold text-error">
      ...
      >
        ...
        {education?.map(edu => (
          <div key={edu.id} className="flex items-center justify-between rounded-xl border border-border-gray bg-light-gray/50 p-4">
            ...
            <button onClick={() => deleteEducation.mutateAsync(edu.id).then(() => { refetchEdu(); refetchProfile(); })} className="p-2 text-error/60 hover:text-error transition-colors">
      ...
      >
        ...
        {experience?.map(exp => (
          <div key={exp.id} className="flex items-center justify-between rounded-xl border border-border-gray bg-light-gray/50 p-4">
            ...
            <button onClick={() => deleteExperience.mutateAsync(exp.id).then(() => { refetchExp(); refetchProfile(); })} className="p-2 text-error/60 hover:text-error transition-colors">
              <Trash2 className="h-4 w-4" />
            </button>
          </div>
        ))}
        ...
        <div className="flex flex-wrap gap-2">
          {skills?.map(skill => (
            <span key={skill.id} className="inline-flex items-center gap-1.5 rounded-full bg-gold/10 text-gold-dark px-3 py-1.5 text-xs font-semibold">
              {skill.name}
              <button onClick={() => deleteSkill.mutateAsync(skill.id).then(() => { refetchSkills(); refetchProfile(); })} className="hover:text-error transition-colors">
