...
export function useVerifyOtp() {
  return useMutation({
    mutationFn: ({ email, otp }: { email: string; otp: string }) =>
      api.post<AuthResponse>('/auth/verify-otp', { email, otp }).then(({ data }: { data: AuthResponse }) => data),
  })
}
...
export function useSetupMfa() {
  return useMutation({
    mutationFn: () =>
      api.get<MfaSetupData>('/auth/mfa/setup').then(({ data }: { data: MfaSetupData }) => data),
  })
}
...
export function useDevices() {
  return useQuery({
    queryKey: ['devices'],
    queryFn: () => api.get<Device[]>('/auth/devices').then(({ data }: { data: Device[] }) => data),
  })
}
...
export function useSessions() {
  return useQuery({
    queryKey: ['sessions'],
    queryFn: () => api.get<Session[]>('/auth/sessions').then(({ data }: { data: Session[] }) => data),
  })
}
